// main/update.mjs — GitHub Releases를 보고 새 버전을 알린다. WHENWORK main/update.mjs를 복사했다(D-14) — 바뀐 건 릴리스 URL과 알림 대상(트레이·설정 창)뿐.
//
...
// 않고, 다음에 앱을 끌 때 설치한다(autoInstallOnAppQuit). 입력줄을 부르려는 순간
// 스스로 재시작하면 그 한 번이 사라진다 — 이 앱에서 그것보다 나쁜 일은 없다.
...
const RELEASES_URL = 'https://github.com/when630/whencommand/releases/latest';
...
export function setupUpdater(ctx) {
  ...
  const notifyRenderer = () => {
    ctx.refreshTray?.();
    ctx.settingsWin?.win?.webContents.send('settings:refresh');
  };
