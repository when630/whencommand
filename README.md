...
- **찾은 것이 없으면 그 글로 할 수 있는 것**이 옵니다 — `메모 검색 “…”`처럼 인자를 받는 형제 앱 명령과, 상단 주석에 `# fallback: yes`를 적은 스크립트(입력 전체가 첫 인자 `$args[0]`·`$1`). 파일 검색 답까지 0개일 때만 보입니다
- **`Ctrl+K`(macOS `⌘K`)** 로 고른 항목의 동작 전부를 봅니다 — 열기 · 파일 위치 열기 · 경로 복사 · 스크립트 열기 · 딥링크 복사. `Ctrl+Enter`는 그중 보조 동작(파일·앱은 폴더에서 보기, 스크립트는 열기)입니다
- **시스템 명령** 다섯이 내장입니다 — `화면 잠금` · `절전` · `휴지통 비우기` · `음소거 전환` · `다크 모드 전환`. 초성(`ㅎㅁㅈㄱ`)으로도 잡힙니다. 종료·재시작은 넣지 않았습니다
